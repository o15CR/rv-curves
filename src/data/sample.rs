...
pub fn generate_sample(snapshot: &FredSnapshot, config: &FitConfig) -> Result<SampleData, AppError> {
    ...
    for i in 0..config.sample_count {
        ...
        let y_obs = base * (sigma_ln * (z + jump) - mean_correction).exp();
...
fn interpolate_bucket_vol(tenor: f64, buckets: &BucketVolatility) -> f64 {
    ...
    if tenor < knots[0].0 {
        ...
        let anchor_vol = knots[0].1;
        let t_safe = tenor.max(0.01);
        return anchor_vol * (t_safe / anchor_tenor).powf(SHORT_END_ALPHA);
    }

    // For long tenors (>= last knot), linear extrapolation.
    if tenor >= knots[knots.len() - 1].0 {
        return linear_interp(knots[knots.len() - 2], knots[knots.len() - 1], tenor);
    }
...
fn bucket_curve(t: f64, buckets: &BucketSeries) -> f64 {
    ...
    if t < knots[0].0 {
        let anchor_tenor = knots[0].0;
        let anchor_spread = knots[0].1;
        // Avoid division by zero; floor tenor at a small value.
        let t_safe = t.max(0.01);
        return anchor_spread * (t_safe / anchor_tenor).powf(SHORT_END_ALPHA);
    }

    // For long tenors (>= last knot), linear extrapolation.
    if t >= knots[knots.len() - 1].0 {
        return linear_interp(knots[knots.len() - 2], knots[knots.len() - 1], t);
    }
